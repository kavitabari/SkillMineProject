...
-----------------------------------------------------------------------------------
alter  table  student  alter column batchid varchar(30)
--3.Append letter ‘_spl’ to all batch ids of trainer ‘Deepa’.update student set batchid=concat('_spl',' ',batchid) where batchid  in(select batchidfrom  student s inner join Course c on c.courseid=s.courseidinner join  Trainer t on  t.trainerid=c.traineridwhere  t.trainername='Deepa')
...
select sname from student where    
  (year (cast(CURRENT_TIMESTAMP as date ))- year  (cast( bithdate as date)))>=30
----------------------------------------------------------
...
alter procedure BalnaceFeeChart( @rollno int ) as
(
select s.sname,(c.totalfees-s.feespaid )as [remaing fee]
from student s inner join Course c on  s.courseid=c.courseid
where  (c.totalfees-s.feespaid )>0 and @rollno=s.rollno  
)
